    ...
    const handleCreate = async () => {
        ...
        try {
            const response = await fetch('http://localhost:3001/api/resumes', {
                ...
                body: JSON.stringify({ title: cvName }),
            });
        ...
        <div className="flex min-h-screen bg-[#FBFBFB] text-slate-900 font-sans">
            ...
            <main className="flex-1 flex flex-col min-w-0">
                ...
                <div className="p-6 lg:p-12 max-w-5xl w-full mx-auto">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {/* New Card (Creative Design) */}
                        <button
                            onClick={() => setIsModalOpen(true)}
                            className="h-64 bg-linear-to-br from-blue-600 to-indigo-700 rounded-4xl p-8 flex flex-col justify-between items-start text-white shadow-xl shadow-blue-100 hover:scale-[1.02] transition-transform relative overflow-hidden group"
                        >
                        ...
                        {/* Existing CV Example Card */}
                        <div className="h-64 bg-white border border-slate-100 rounded-4xl p-6 flex flex-col shadow-sm hover:shadow-md transition-all">
                            <div className="flex-1 bg-slate-50 rounded-2xl mb-4 flex items-center justify-center border border-slate-50">
                                <Layout size={40} className="text-slate-200" />
                            </div>
                            <div className="flex justify-between items-center">
                                <div>
                                    <h3 className="font-bold">Software Engineer CV</h3>
                                    <span className="text-[11px] text-slate-400 font-bold uppercase flex items-center gap-1">
                                        <Clock /> Edited yesterday
                                    </span>
                                </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-white/80 backdrop-blur-xl animate-in fade-in duration-300" onClick={() => setIsModalOpen(false)}></div>
                    ...
                    <div className="relative bg-white w-full max-w-lg rounded-[2.5rem] p-10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.1)] border border-slate-100 animate-in zoom-in-95 duration-300">
                        <button onClick={() => setIsModalOpen(false)} className="absolute top-8 right-8 text-slate-400 hover:text-black transition-colors">
                        ...
                        <div className="space-y-8">
                            <div>
                                ...
                                <p className="text-slate-500 font-medium leading-relaxed">Give your project a name. This is only visible to you on your dashboard.</p>
                            </div>
                            ...
                            <div className="space-y-2">
                                <input
                                    ...
                                    placeholder="Resume Title (e.g. Google Application)"
                            ...
                            <button
                                ...
                                className="w-full py-5 bg-black text-white rounded-2xl font-bold text-lg hover:bg-slate-800 disabled:bg-slate-200 disabled:text-slate-400 transition-all shadow-xl shadow-slate-200 active:scale-[0.98]"
                            >
                                Start Building
                            </button>
                        </div>
